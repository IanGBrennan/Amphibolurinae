...
testa <- dist.nodes(phy=agam.tree, trait.df=LSR.anc, metric="RootParentChild", distance="euclidean")

mv.fit


mv.bm.fit <- mvBM(tree=agam.tree, data=all.LSR.traits, model="BM1",
                  param=list(constraint="diagonal"))
mvMORPH::mvSIM(agam.tree, nsim=1, model="BM1")

sigmas <- NULL; thetas <- NULL
for(k in 1:ncol(all.LSR.traits)){
  curr.fit <- mvBM(tree=agam.tree, data=dplyr::select(all.LSR.traits, colnames(all.LSR.traits)[[k]]), model="BM1")
  sigmas <- append(sigmas, curr.fit$sigma)
  thetas <- append(thetas, curr.fit$theta)
}

testo <- matrix(data=0, nrow=19, ncol=19)
diag(testo) <- sigmas

mv.bm.sim <- data.frame(mvSIM(tree=agam.tree, nsim=1, model="BM1",
                   param=list(theta=thetas, sigma=testo, ntraits=19)))

testa <- dist.nodes(phy=agam.tree, trait.df=)
...
for(j in 1:nsim){
  # simulate a set of data
  sim.df <- data.frame(sapply(1:19, function(x) fastBM(tree=agam.tree, a=thetas[x], sig2=sigmas[x], internal=T)))
  # fix the rownames for the internal nodes
  rownames(sim.df)[120:237] <- paste0("n", rownames(sim.df)[120:237])
  # estimate the multivariate trait change
  testi <- dist.nodes(phy=agam.tree, trait.df=sim.df, metric="RootParentChild", distance="euclidean")
  # keep just the distance measured
  distances <- cbind(distances, testi$distance)
  # let me know where we are
  print(j)
}
distances <- data.frame(distances)
...
apply.quantile <- function(x){quantile(x,probs=c(0.05,0.95))}
sim.ci <- data.frame(t(apply(distances, 1, apply.quantile)))
colnames(sim.ci) <- c("lower","upper")
sim.ci$mean <- apply(sim.ci, 1, mean)
full.ci <- cbind(testa, sim.ci)
full.ci$distance.abs <- abs(full.ci$distance)
...
plot.phylo(agam.tree, edge.color = unlist(full.ci$edge.color), edge.width=2, type="fan", cex=0.4, open.angle=5, no.margin=T)


res.l <- dplyr::filter(full.ci, results == "less")
...
# subset the shifts that are greater than expected
res.g <- dplyr::filter(full.ci, results == "greater")
# process the evolvability shifts if there are any
if(nrow(res.g) > 0){
  # rescale color of the evolvability branches by rate
  for(k in 1:nrow(res.g)){
    curr.edge <- res.g[k,]
...
full.ci$xstart <- sapply(full.ci$name.parent, function(x) pca.x[which(rownames(pca.x)==x),"PC1"])
full.ci$ystart <- sapply(full.ci$name.parent, function(x) pca.x[which(rownames(pca.x)==x),"PC2"])
full.ci$xstop  <- sapply(full.ci$name.child,  function(x) pca.x[which(rownames(pca.x)==x),"PC1"])
full.ci$ystop  <- sapply(full.ci$name.child,  function(x) pca.x[which(rownames(pca.x)==x),"PC2"])




ggplot() +
  geom_point(data=full.ci, aes(x=xstop,y=ystop),color="grey") +
  geom_segment(data=dplyr::filter(full.ci, results=="greater" & distance > 0), aes(x=xstart,y=ystart,xend=xstop,yend=ystop), arrow=arrow(), color="blue") +
  geom_segment(data=dplyr::filter(full.ci, results=="greater" & distance < 0), aes(x=xstart,y=ystart,xend=xstop,yend=ystop), arrow=arrow(), color="red") +
#  geom_point(data=dplyr::filter(full.ci, results=="less"), aes(x=xstop,y=ystop), color="black", size=3) +
  theme_classic()
