...
trait.at.time <- function(timeslices, phy, trait.vector, plot=F){
  ...
  trait.vector <- trait.vector[match(c(phy$tip.label, phy$node.label), names(trait.vector))]
...
trait.at.time.multi <- function(timeslices, phy, trait.matrix, plot=F){
  ...
  for (k in 1:ncol(trait.matrix)){
    ...
    curr.trait <- trait.at.time(timeslices=timeslices, phy=phy, trait.vector=trait.vector, plot=plot)
...
agg.func.eve <- function(x) dispRity::func.eve(as.matrix(c(x)))

############################################################################

extract.variance <- function(trait.time.obj, plot=c("cumulative", "relative", "sideXside", FALSE), metric=c("variance", "functional divergence", "disparity", "functional eveneness")){
  ...
  if(metric == "functional evenness"){
    time.variance$measure <- aggregate(trait.time.obj[,2], list(trait.time.obj$time), agg.func.eve)[,2]
    extant.var <- max(time.variance$measure)
  }
...
extract.volume <- function(obj, PCs, parallel, plot=T){
  ...
  message(paste("fitting hypervolume to", PCs, "dimensions across", length(utime), "timeslices. This could take a while."))
  for(j in 1:length(utime)){
    curr.traits <- dplyr::filter(obj, time==utime[[j]])
    if(!nrow(curr.traits) >= PCs){next}
    tto.list[[j]] <- curr.traits
    names(tto.list)[[j]] <- paste0("time_",utime[[j]])
  }
  # remove list elements that are null (have fewer than specified PCs/samples)
  tto.list <- tto.list[which(!names(tto.list)=="NULL")]
  
  # Run the PCA and hypervolume estimation across each timeslice (will be slow)
  hv <- parallel::mclapply(tto.list, function(x) reduce.hypervolume(x, PCs), mc.cores = parallel)
  
  #
  volumes <- lapply(hv, hypervolume::get_volume)
  vdf <- data.frame(time = sapply(names(volumes), function(x) strex::str_after_first(x,"_")),
                    volume = unlist(volumes))
  vdf$time <- as.numeric(vdf$time)
  vdf$time.rev <- vdf$time - max(vdf$time)
  vdf$volume <- as.numeric(vdf$volume)
...
# example:
# extract.volume(trait.time.object, PCs=3, parallel=8, plot=F)
...
order.by.phylo <- function(jumbled.order, phy, nodelabels=F){
  # make sure the matrix order matches the tip + node order
  
  # if the data is in a vector (single trait)
  if(is.vector(jumbled.order) & nodelabels==F){
  ...
  # if the data is in a matrix or data frame (multiple traits) and tree has nodelabels
  if(!is.vector(jumbled.order) & nodelabels==T){
  phylo.order <- jumbled.order[order(match(rownames(jumbled.order), 
...
# This function will run the funspace command with plots grouped by timeslices

# note, when princomp(x, cor=F) it is identical to prcomp(x)
# best to use cor=F when using rescaled data like Log-Shape Ratios 
# see: https://stats.stackexchange.com/questions/242864/princomp-outputs-seemingly-wrong-pca-scores-with-cor-true-input-argument
funspace.at.time <- function(timeslices, phy, trait.df, pcs, plot=F, corr=T, smooth=100){
  
  # reorder the data and run pca on the reordered dataframe
  trait.df.order <- order.by.phylo(trait.df, phy)
  trait.pca <- princomp(trait.df.order, cor=corr)
  ...
  # define the timeslices for extracting trait values, and apply it to your tree
  ts <- c(seq(from=0, to=root2, by=timeslices), root2)
  tsrev <- round(root2 - ts, digits=2)
  
  # get the number of traits in the original dataset
  ntraits <- ncol(trait.df)
  
  # combine the trait values with the node ages (including tips)
  ndel <- node.depth.edgelength(phy) # age of each node (tips are largest numbers)
  pca.ages <-  cbind(trait.pca$scores, ndel) # seting reconstructed phenotype with x coordinate (age) of corresponding node
  # new is just head.anc[,1]
  ...
  for (i in 1:length(ts)) {      
    ...
    # extract the rates for the branches of interest
    curr.traits <- trait.df.order[curr.edge.nodes[,2],]
    
    # extract trait values and ages at start/end nodes
    pca.start <- pca.ages[curr.edge.nodes[,1],]
    pca.end   <- pca.ages[curr.edge.nodes[,2],]
    
    # apply the current slice time
    slice <- rep(ts[i],times = length(pca.end[,1]))
    slice.rev <- rep(tsrev[i],times = length(pca.end[,1]))
    
    # estimate the pca values along the edges at timeslice i
    #  (delta trait / delta time) * (edge fragment length) + (original trait value)
    pca <- (((pca.end-pca.start)/(pca.end[,"ndel"]-pca.start[,"ndel"])) * 
              (ts[i] - pca.start[,"ndel"])) + pca.start
    
    # add the pca values at timeslice i to a dataframe of all values
    all.time.pca <- rbind(all.time.pca, pca[,1:ntraits])

    # build a vector of the groups based on the timeslice
    group.vec <- append(group.vec, rep(tsrev[[i]], nrow(curr.edge.nodes)))
    
  }
  
  # generate a funspace for each timeslice
  funtime <- funspace(x = all.time.pca, PCs = pcs, group.vec = group.vec, n_divisions = smooth)
  # correct the objectClass to specify we've used PCA values
  funtime$parameters$objectClass <- "princomp"

  # make informative axis labels
  propV <- 100 * (trait.pca$sdev)**2 / sum((trait.pca$sdev)**2)
  axis.t.x <- paste0("PC", funtime$parameters$PCs[1], " (", round(propV[funtime$parameters$PCs[1]], 2),"%", ")")
  axis.t.y <- paste0("PC", funtime$parameters$PCs[2], " (", round(propV[funtime$parameters$PCs[2]], 2),"%", ")")


  # plot all the slices if you'd like to
  if(plot==T){plot(x = funtime, 
                   type = "groups", 
                   quant.plot = T, 
                   globalContour = T, 
                   fixed.bw = T,
                   colors = brewer.pal(5, "YlOrRd"),
                   axis.title.x = axis.t.x,
                   axis.title.y = axis.t.y)}
...
# best to use cor=F when using rescaled data like Log-Shape Ratios 
# see: https://stats.stackexchange.com/questions/242864/princomp-outputs-seemingly-wrong-pca-scores-with-cor-true-input-argument
funspace.through.time <- function(timeslices, phy, trait.df, pcs, plot=F, corr=T, 
                                  smooth=100, save.img=F, quantile=0.999, tpd=T){
  
  message(paste("Plotting the functional space at specified timeslices. Thick perimeter line denotes the",quantile,"trait quantiles."))
  ...
  # make informative axis labels
  propV <- 100 * (trait.pca$sdev)**2 / sum((trait.pca$sdev)**2)
  axis.t.x <- paste0("PC", pcs[[1]], " (", round(propV[pcs[[1]]], 2),"%", ")")
  axis.t.y <- paste0("PC", pcs[[2]], " (", round(propV[pcs[[2]]], 2),"%", ")")
  xmax <- max(pca.ages[,pcs[1]]) * 2 #xmax <- max(pca.ages[,pcs[1]]) + (0.25*max(pca.ages[,pcs[1]]))
  xmin <- min(pca.ages[,pcs[1]]) * 2 #xmin <- min(pca.ages[,pcs[1]]) - (0.25*min(pca.ages[,pcs[1]]))
  ymax <- max(pca.ages[,pcs[2]]) * 2 #ymax <- max(pca.ages[,pcs[2]]) + (0.25*max(pca.ages[,pcs[2]]))
  ymin <- min(pca.ages[,pcs[2]]) * 2 #ymin <- min(pca.ages[,pcs[2]]) - (0.25*min(pca.ages[,pcs[2]]))

  # set colors if plotting just the global contour
  col.ramp <- colorRampPalette(rev(RColorBrewer::brewer.pal(9, "Spectral")))
  col.ramps <- col.ramp(length(ts))
  ...
  for (i in 1:length(ts)) {      
    ...
    # which edges (row number of nH) overlap timeslice i
    curr.edges <- intersect(which(nH[,1]<= ts[i] ), which(nH[,2] >= ts[i]))
    if(length(curr.edges) <= 2){warning(paste("timeslice", i, "has less than 3 edges. Skipping to next slice.")); next}
    ...
    # generate a funspace for each timeslice
    funtime <- funspace(x = pca[,1:ntraits], PCs = pcs, n_divisions = smooth, threshold = quantile)
    # correct the objectClass to specify we've used PCA values
    funtime$parameters$objectClass <- "princomp"

    # set the color palette, depending on if we want to plot the TPD
    if(tpd==T){col.pal = brewer.pal(5, "YlOrRd")}
    if(tpd==F){col.pal = c(NA,NA)}
    
    # plot all the slices if you'd like to
    if(plot==T && save.img==F && tpd==T){
      plot(x = funtime, 
                     type = "global", 
                     quant.plot = T, 
                     quant = quantile,
                     ...
                     xlim = c(xmin,xmax),
                     ylim = c(ymin,ymax))
      title(paste("Timeslice:", tsrev[[i]]))}

    # plot all the slices if you'd like to
    if(plot==T && save.img==F && tpd==F){
      plot(x = funtime, 
                     type = "global", 
                     quant.plot = T, 
                     quant = quantile,
    ...
    par(new=T)
          ...
          # plot all the slices if you'd like to
    if(plot==T && save.img==T){
      img.name <- paste0(getwd(),"/",paste0("Timeslice_",tsrev[[i]]),".jpg")
      jpeg(filename=img.name, quality=100, units="in", width=8, height=8, res=150)
      #jpeg(filename=img.name)
      plot(x = funtime, 
                     type = "global", 
                     quant.plot = T,
                     quant = quantile,
