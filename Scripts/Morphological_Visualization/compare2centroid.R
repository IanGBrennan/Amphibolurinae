...
# make a column for genus name
allLSR$Genus <- sapply(allLSR$Genus_species, function(x) strsplit(x,"_")[[1]][1])
...
# move each taxon into an appropriate regime dataset
e.R1 <- dplyr::filter(allLSR, Genus %in% c("Hypsilurus","Lophosaurus","Chelosania","Physignathus"))
e.R2 <- dplyr::filter(allLSR, Genus %in% c("Tympanocryptis"))
e.R3 <- dplyr::filter(allLSR, Genus %in% c("Amphibolurus","Gowidon","Tropicagama","Diporiphora","Lophognathus",
                                           "Pogona","Rankinia","Chlamydosaurus","Intellagama"))
...
all.compare <- allLSR
all.compare$dist2R1 <- compare2centroid(e.R1[,1:19], allLSR[,1:19])
all.compare$dist2R2 <- compare2centroid(e.R2[,1:19], allLSR[,1:19])
all.compare$dist2R3 <- compare2centroid(e.R3[,1:19], allLSR[,1:19])
