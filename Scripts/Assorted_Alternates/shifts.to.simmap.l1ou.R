...
#shifts.to.simmap.l1ou <- function (l1ou.object) {
#  tree <- l1ou.object$tree
#  shifts_edges <- l1ou.object$shift.configuration
#  
#  if (!requireNamespace("phytools", quietly = TRUE)) {
#    stop("Package 'phytools' is needed for function 'shifts.to.simmap.l1ou'. Please install it.", 
#         call. = FALSE)
#  }
...
#  daughters <- sort(shifts_edges)
#  daughter.nodes <- sort(sapply(daughters, function(x) tree$edge[x,2]))
#      daughter.nodes <- c(daughter.nodes[which(daughter.nodes >  Ntip(tree))],
#                          daughter.nodes[which(daughter.nodes <= Ntip(tree))])
#  for (i in 1:length(daughter.nodes)) {
#    curr.node <- daughter.nodes[[i]]
#    tree <- phytools::paintSubTree(tree, curr.node, state = i, 
...
shifts.to.simmap.l1ou <- function(l1ou.object){
  tree <- l1ou.object$tree; base <- tree
  if(is.null(names(l1ou.object$shift.configuration))){
      shifts_edges <- data.frame(regime=1:length(l1ou.object$shift.configuration),
                             edge=l1ou.object$shift.configuration)
  }
  else{
      shifts_edges <- data.frame(regime=names(l1ou.object$shift.configuration),
                             edge=l1ou.object$shift.configuration)
  }
  daughters <- shifts_edges
  daughters$node <- tree$edge[daughters$edge,2]
  daughters$regime <- as.numeric(daughters$regime)+1
  
  desc.tips <- NULL
  for(k in 1:nrow(daughters)){
    curr.row <- daughters[k,]
    if(curr.row$node > Ntip(tree)){desc.tips <- append(desc.tips, Ntip(extract.clade(base, node=curr.row$node)))}
    else{desc.tips <- append(desc.tips, 1)}
  }
  daughters$descendants <- desc.tips
  daughters <- daughters[order(daughters$descendants, decreasing=T),]
  
  for (i in 1:nrow(daughters)) {
    curr.node <- daughters[i,]
    if(curr.node$node <= Ntip(tree)){tree <- phytools::paintBranches(tree, curr.node$node, state = curr.node$regime)}
    if(curr.node$node  > Ntip(tree)){tree <- phytools::paintSubTree(tree, curr.node$node, state = curr.node$regime, stem = TRUE)}
    #plotColorSimmap(tree)
  }
  return(tree)
}


# new version of plotSimmap with additional colors
require(randomcoloR)
plotColorSimmap <- function (tree, colors = NULL, fsize = 1, ftype = "reg", lwd = 2, 
    ...
    lend = 2, asp = NA, outline = FALSE, plot = TRUE) 
{
    if (inherits(tree, "multiPhylo")) {
        ...
        if (is.null(colors)) {
            ...
            colors <- randomColor(length(st)-1)
        ...
        if (type == "phylogram") {
            if (direction %in% c("upwards", "downwards")) {
                ...
                if (outline) {
                  ...
                  black <- colors
                  black[] <- "black"
                  phytools::plotPhylogram(tree, colors = black, fsize, 
                    ftype, lwd = lwd + 2, pts, node.numbers, 
                    mar, add, offset, direction, setEnv, xlim, 
                    ylim, nodes, tips, split.vertical, lend, 
                    asp, plot)
            ...
            if (outline) {
                ...
                plotFan(tree, colors = black, fsize, ftype, lwd = lwd + 
                  2, mar, add, part, setEnv, xlim, ylim, tips, 
...
# Bunch of dependency internal functions from:
# https://github.com/liamrevell/phytools/blob/master/R/plotSimmap.R
