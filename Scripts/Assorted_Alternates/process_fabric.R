...
# setwd("/Applications/BayesTraitsV4/Tiliquini_Fabric_Traits")
# phy <- read.nexus("Tiliquini_BT.tre")
# res.files <- dir(".", ".csv")
 trait.files <- dir(".", ".tx")

process_FPP <- function(res.files, phy, trait.files, col.palette){
  # make some empty objects for us to store results
  beta.trees <- NULL; scalar.trees <- NULL; all.res <- NULL
  ...
  for (p in 1:length(res.files)){
    ...
    # choose 
    colnames(BTres)[c(22,12)] <- c("Sig_Beta", "Mean_Beta", )
